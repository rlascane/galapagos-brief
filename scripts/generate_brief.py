...
# ---------------------------------------------------------------------------
# Extração de texto + citações
# ---------------------------------------------------------------------------

def extract_brief(response: anthropic.types.Message) -> tuple[str, list[dict]]:
    """
    Concatena todos os blocos de texto na ordem em que apareceram.
    Coleta citações únicas (por URL) para renderizar como footnotes.
    """
    text_parts: list[str] = []
    citations: list[dict] = []
    ...
    for block in response.content:
        if block.type == "text":
            ...
            for cit in block_citations:
                # cit pode ser dict ou objeto pydantic; normalizar
...
def main() -> int:
    if not os.environ.get("ANTHROPIC_API_KEY"):
        print("ERRO: ANTHROPIC_API_KEY não definido.", file=sys.stderr)
        return 1
    ...
    if not body:
        print("ERRO: resposta sem conteúdo de texto.", file=sys.stderr)
        return 2

    # Token usage para o log do Actions
    usage = response.usage
    print(
        f"[generate_brief] tokens: input={usage.input_tokens} "
        f"output={usage.output_tokens} | citações: {len(citations)}"
    )
